...
<div class="row cart-wrapper">
	<div class="col-md-12 col-lg-12 col-xl-9">
		<h2><?php esc_html_e('Cart', 'understrap-child'); ?></h2>
							...
							<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?>">
								...
								<td class="product-thumbnail">
									...
									if (!$product_permalink) {
										echo wp_get_attachment_image($_product->get_image_id(), $image_size); // PHPCS: XSS ok.
									} else {
										printf('<a href="%s">%s</a>', esc_url($product_permalink), wp_get_attachment_image($_product->get_image_id(), $image_size)); // PHPCS: XSS ok.
									}
								...
								<td class="product-price" data-title="<?php esc_attr_e('Price', 'woocommerce'); ?>">
									...
									// If the product is a variation, get the variation's prices instead
									if ($_product->is_type('variation')) {
										$variation = new WC_Product_Variation($_product->get_id());
										$regular_price = $variation->get_regular_price();
										$sale_price = $variation->get_sale_price();
									}

									// If the sale price exists and is less than the regular price, show the sale price instead
									if ($sale_price && $sale_price < $regular_price) {
										$price = wc_price($sale_price);
										$regular_price = wc_price($regular_price);
										echo '<div class="cart-regular-price">' . $regular_price . '</div> <div class="cart-sale-price">' . $price . '</div>';
									} else {
										$price = wc_price($regular_price);
										echo '<div class="cart-sale-price">' . $price . '</div>';
									}
									?>
								</td>
